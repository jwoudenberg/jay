...
node = |tag_name, attributes, children|
    open_tag =
        Str.to_utf8 "<${tag_name}"
...
text : Str -> Xml
text = |str|
    bytes = Xml.Attributes.escape (Str.to_utf8 str) [
        ...
        when snippet is
            RocGenerated bytes ->
                when Str.from_utf8 bytes is
                    Ok str -> str
                    Err err -> crash "Failed to convert bytes to Str: ${Inspect.to_str err}"

            FromSource { start, end } ->
                "{SOURCE:${Num.to_str start}:${Num.to_str end}}"
    |> Str.join_with ""
